...
const HeroSection = () => {
  ...
  }, [slidesLength, hovered]); // Added 'hovered' as a dependency
  ...
  return (
    // Enhanced background gradient and height adjustment
    <section className="w-full bg-gradient-to-br from-gray-100 via-gray-200 to-gray-300 h-[90vh] sm:h-[75vh] px-4 py-8 dark:from-slate-800 dark:via-slate-900 dark:to-black">
      <div className="mx-auto grid max-w-7xl h-full grid-cols-1 gap-6 md:grid-cols-3">
        {/* Main Carousel Card - Enhanced shadow and background */}
        <div
        ...
        >
          {/* Navigation Buttons - Enhanced hover effects and added dark mode styles */}
          <div
            className={`absolute inset-0 z-10 flex items-center justify-between px-2 sm:px-4 transition-opacity duration-300 ${
              hovered ? "opacity-100" : "opacity-0 md:opacity-0" // md:opacity-0 keeps them hidden on medium+ screens unless hovered
            }`}
          >
            <button
              onClick={prevSlide}
          ...
          {/* Slide Content - Added custom animation class 'animate-fade-slide-in' and leading-relaxed */}
          {/* NOTE: 'animate-fade-slide-in' requires configuration in tailwind.config.js */}
          {/* Added padding to content div to ensure space around text */}
          <div key={current} className="flex-1 text-center md:text-left transition-all duration-700 ease-out animate-fade-slide-in p-4 md:p-0">
            ...
            {/* Changed dark mode text color for consistency/appeal, added text-balance */}
            ...
            {/* Button styling is already good, added slightly more emphasis on focus */}
          ...
          {/* Slide Image - Kept original animation, could use fade-slide-in as well */}
          {/* Added padding to image container to ensure space around image */}
          <div className="h-56 w-56 md:h-72 md:w-72 lg:h-80 lg:w-80 transition-all duration-700 ease-in-out flex items-center justify-center p-4 md:p-0">
            <img
              ...
              className="h-full w-full object-contain mx-auto animate-fadeIn" // Or animate-fade-slide-in
              role="img"
            />
          </div>

          {/* Dot Indicators - Added dark mode active and hover colors */}
        ...
        {/* Side Cards - Added hover:shadow-xl for more consistent visual feedback */}
        <div className="flex flex-col gap-6">
          {/* Card 1 - Enhanced shadow */}
          <div className="flex items-center gap-4 rounded-xl bg-white p-5 shadow-xl hover:shadow-2xl hover:scale-[1.03] transition-all duration-300 dark:bg-slate-800">
            <div className="flex-1">
              ...
              {/* Changed dark mode text color for consistency/appeal */}
              <h2 className="mt-1 text-base font-bold text-gray-700 dark:text-teal-300 lg:text-lg">
                iPhone 12 Pro Max {/* Example product, keep as is or make dynamic */}
              </h2>
              <p className="mt-1.5 text-lg font-bold text-indigo-700 dark:text-indigo-400">
                EGP 12,480.00 {/* Example price */}
              </p>
            </div>
            <img
              src={hero5} // Using hero5 for example, replace with appropriate image
              alt="iPhone 12 Pro Max"
              className="w-16 object-contain md:w-20 lg:w-24"
            />
          </div>

          {/* Card 2 - Enhanced shadow */}
