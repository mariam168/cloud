...
const HeroSection = ({ serverUrl = 'http://localhost:5000' }) => {
    ...
    const [sideOffersData, setSideOffersData] = useState({}); // Will now be an object: { iphoneOffer: {}, weeklyOffer: {} }
    ...
    useEffect(() => {
        const fetchHeroData = async () => {
            ...
            try {
                ...
                // Fetch side offers using the new dedicated endpoint
                const sideOffersRes = await axios.get(`${serverUrl}/api/hero-side-offers`);
                setSideOffersData(sideOffersRes.data); // This now expects { iphoneOffer: {}, weeklyOffer: {} }
    ...
    console.log("DEBUG: Current Slide ID for Link:", currentSlideContent?._id);
    console.log("DEBUG: Current Slide Content:", currentSlideContent); // Add this line as well

    const formatCurrency = (amount) => {
        // Simple currency format, adjust as needed for actual currencies
        ...
        <section className="w-full bg-gradient-to-br from-gray-100 via-gray-200 to-gray-300 h-auto py-12 px-4 transition-colors duration-500 ease-in-out dark:from-slate-800 dark:via-slate-900 dark:to-black sm:py-16 md:py-20 lg:py-24 overflow-hidden">
            <div className="mx-auto grid max-w-7xl h-full grid-cols-1 gap-8 md:grid-cols-3">
                {currentSlideContent && (
                    ...
                    >
                        {/* Adjusted z-index for navigation arrows overlay to be lower than content */}
                        {slidesLength > 1 && (
                            <div
                                className={`absolute inset-0 z-[5] flex items-center justify-between px-4 transition-opacity duration-300 ${hovered ? "opacity-100" : "opacity-0 md:opacity-100"
                                    }`}
                            >
                                <button
                                    onClick={prevSlide}
                        ...
                        {/* Added z-index to content div to ensure it's above the navigation overlay */}
                        <div className="relative z-[8] flex flex-col md:flex-row items-center justify-center flex-1 text-center md:text-left transition-all duration-700 ease-out p-4 md:p-0">
                            ...
                            <div className="flex-1 ml-0 md:ml-8 p-4 md:p-6 lg:p-8 order-2 md:order-1">
                                ...
                                {/* This is your "Shop Now" button, with temporary borders for debugging removed */}
                                <Link to={`/advertisements/${currentSlideContent._id}`} className="inline-block">
                                    <button
                                        // Adding a direct onClick for final confirmation (can remove after verification)
                                        onClick={() => console.log("Shop Now button clicked (final check)! Navigating to:", `/advertisements/${currentSlideContent._id}`)}
                                        className="mt-6 rounded-lg bg-blue-900 px-8 py-3 text-base font-semibold text-white shadow-md hover:bg-teal-700 transition-transform transform hover:scale-105 focus:outline-none focus:ring-2 focus:ring-teal-500 focus:ring-opacity-50"
                                    >
                            ...
                            <div className="flex items-center gap-4 rounded-xl bg-white p-6 shadow-xl hover:shadow-2xl hover:scale-[1.03] transition-all duration-300 dark:bg-slate-800 border border-gray-200 dark:border-slate-700 cursor-pointer">
                                <div className="flex-1">
                                    ...
                                    <p className="mt-1.5 text-lg font-bold text-indigo-700 dark:text-indigo-400">
                                        {sideOffersData.iphoneOffer.price ? formatCurrency(sideOffersData.iphoneOffer.price) : 'N/A'}
                                    </p>
                                </div>
                                <img
