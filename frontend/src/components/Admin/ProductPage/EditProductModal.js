...
const EditProductModal = ({ product, onClose, onProductUpdated, serverUrl }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!editedProduct.name_en || editedProduct.name_en.trim() === "" || !editedProduct.name_ar || editedProduct.name_ar.trim() === "" || editedProduct.price === undefined || editedProduct.price === null || Number(editedProduct.price) < 0) {
        setErrorMessage("Product English and Arabic names, and a valid price are required.");
  ...
  return (
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
      <div className="bg-white p-6 sm:p-8 rounded-lg shadow-xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
        <h2 className="text-2xl font-bold mb-6 text-center text-gray-700">Edit Product</h2> 
        {errorMessage && <p className="text-red-500 bg-red-100 p-3 rounded mb-4 text-sm">{errorMessage}</p>}
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label htmlFor="edit-name_en" className="block text-sm font-medium text-gray-700">Product Name (English)</label>
            <input
              type="text"
              id="edit-name_en"
              name="name_en"
          ...
          <div>
            <label htmlFor="edit-name_ar" className="block text-sm font-medium text-gray-700">اسم المنتج (العربية)</label>
            <input
              type="text"
              id="edit-name_ar"
              name="name_ar"
              ...
              onChange={handleChange}
              className="mt-1 w-full p-3 border border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 text-right" // Added text-right for Arabic input
              required
            />
          </div>
          <div>
            <label htmlFor="edit-description_en" className="block text-sm font-medium text-gray-700">Description (English)</label>
            <textarea
          ...
          <div>
            <label htmlFor="edit-description_ar" className="block text-sm font-medium text-gray-700">الوصف (العربية)</label>
            <textarea
          ...
          <div>
            <label htmlFor="edit-image" className="block text-sm font-medium text-gray-700">Product Image (Optional to change current)</label>
            {currentImageUrl && (
