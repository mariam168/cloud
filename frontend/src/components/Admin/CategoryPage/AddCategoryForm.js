// frontend/src/components/Admin/CategoryPage/AddCategoryForm.js
...
const AddCategoryModal = ({ isOpen, onClose, onCategoryAdded, serverUrl }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        if (!name.trim()) {
            setMessage({ text: t.adminCategoryPage?.categoryNameRequiredError || 'اسم الفئة مطلوب.', type: 'error' });
            return;
        }
        ...
        try {
            ...
            setMessage({ text: err.response?.data?.message || (t.adminCategoryPage?.errorAddingCategory || "حدث خطأ أثناء إضافة الفئة."), type: 'error' });
        } finally {
        ...
        <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center p-4 z-50 transition-opacity duration-300 ease-in-out">
            <div className="bg-white p-6 sm:p-8 rounded-lg shadow-xl w-full max-w-md max-h-[90vh] overflow-y-auto transform transition-all duration-300 ease-in-out scale-100">
                <div className="flex justify-between items-center mb-6">
                    ...
                    <h3 className="text-2xl font-bold text-gray-700">{t.adminCategoryPage?.addCategoryModalTitle || 'إضافة فئة جديدة'}</h3>
                    <button
                        onClick={onClose}
                        className="text-gray-400 hover:text-gray-600 transition-colors p-1 rounded-full hover:bg-gray-100"
                        // Use translated aria-label
                        aria-label={t.adminCategoryPage?.closeButtonLabel || "إغلاق"}
                    >
                ...
                <form onSubmit={handleSubmit} className="space-y-5">
                    ...
                    <div>
                        ...
                        <label htmlFor="modal-category-description" className="block text-sm font-medium text-gray-700 mb-1">
                            {t.adminCategoryPage?.categoryDescriptionLabel || 'وصف الفئة (اختياري)'}
                        </label>
                        <textarea
                    ...
                    <div className="flex justify-end space-x-3 pt-4">
                        ...
                        >
                            ...
                            {isSubmitting ? (t.adminCategoryPage?.addingButton || "جاري الإضافة...") : (t.adminCategoryPage?.addButton || "إضافة الفئة")}
                        </button>
                    </div>
                </form>
            </div>
