...
const LoginPage = () => {
    ...
    const onSubmit = async e => {
        ...
        try {
            ...
            navigate('/');
            alert(t('loginPage.loginSuccess') || 'Login successful!'); // Translate success message
        } catch (err) {
            ...
            if (errors && Array.isArray(errors)) {
                ...
                setError(t('loginPage.loginFailed') || 'Login failed. Please check your credentials.'); // Translate generic error
            }
        ...
        <div className="min-h-screen flex items-center justify-center bg-gradient-to-r from-indigo-100 to-purple-200 px-4 dark:from-gray-800 dark:to-gray-900 transition-colors duration-300 ease-in-out">
            <div className="w-full max-w-md bg-white dark:bg-gray-700 p-8 rounded-2xl shadow-2xl transition-colors duration-300 ease-in-out">
                <h1 className="text-3xl font-bold text-center text-indigo-700 dark:text-indigo-400 mb-6 transition-colors duration-300 ease-in-out">
                    {t('loginPage.title') || 'Login to Your Account'} {/* Translate title */}
                </h1>
                ...
                <form onSubmit={onSubmit} className="space-y-5">
                    <div>
                        <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                            {t('loginPage.emailLabel') || 'Email'} {/* Translate label */}
                        </label>
                        <input
                            type="email"
                            name="email"
                    ...
                    <div>
                        <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                            {t('loginPage.passwordLabel') || 'Password'} {/* Translate label */}
                        </label>
                        <input
                            type="password"
                            name="password"
                    ...
                    >
                        {loading ? (t('loginPage.loggingInButton') || 'Logging in...') : (t('loginPage.loginButton') || 'Login')} {/* Translate button text */}
                    </button>
                </form>

                <p className="mt-6 text-center text-sm text-gray-600 dark:text-gray-400">
                    {t('loginPage.noAccount') || 'Don’t have an account?'}{' '} {/* Translate text */}
                    <Link to="/register" className="text-indigo-600 hover:text-indigo-500 dark:text-indigo-400 dark:hover:text-indigo-300 font-medium transition-colors duration-200 ease-in-out">
                        {t('loginPage.signUpLink') || 'Sign up here'} {/* Translate link text */}
                    </Link>
                </p>
            </div>
        </div>
