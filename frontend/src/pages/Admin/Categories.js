...
const AdminCategoriesPage = () => {
    ...
    const handleOpenEditForm = async (category) => {
        try {
            ...
            alert(error.response?.data?.message || 'حدث خطأ أثناء جلب تفاصيل التصنيف');
        }
    };
                            ...
                            <div key={cat._id}>
                                ...
                                >
                                    <div className="flex items-center gap-4 flex-grow">
                                        ...
                                        {cat.imageUrl ? (
                                            <img src={`${serverUrl}${cat.imageUrl}`} alt={cat.name} className="w-12 h-12 rounded-md object-cover" />
                                        ) : (
                                        ...
                                        <ul className="divide-y divide-gray-200 border-l-2 border-gray-200 pl-4">
                                            {cat.subCategories.map(sub => (
                                                <li key={sub._id} className="flex items-center py-3">
                                                    {sub.imageUrl ? (
                                                        <img src={`${serverUrl}${sub.imageUrl}`} alt={sub.name} className="w-10 h-10 rounded-md object-cover mr-3" />
                                                    ) : (
