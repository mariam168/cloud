...
import { Loader2, CheckCircle, XCircle, Truck, User, Package, Calendar, ChevronLeft, Info, ReceiptText } from 'lucide-react';
...
const AdminOrderDetailsPage = () => {
    ...
    const fetchOrderDetails = useCallback(async () => { setLoading(true); setError(null); try { const config = { headers: { Authorization: `Bearer ${token}` } }; const response = await axios.get(`${API_BASE_URL}/api/orders/${id}`, config); setOrder(response.data); } catch (err) { const errorMessage = t('adminOrdersPage.errorFetchingOrderDetails'); setError(errorMessage); showToast(errorMessage, 'error'); } finally { setLoading(false); } }, [id, token, API_BASE_URL, t, showToast]);
    useEffect(() => { if (loadingAuth) return; if (!isAuthenticated || !currentUser || currentUser.role !== 'admin') { navigate('/'); showToast(t('general.accessDenied'), 'error'); return; } if (token && API_BASE_URL && id) { fetchOrderDetails(); } }, [id, isAuthenticated, currentUser, token, API_BASE_URL, navigate, t, loadingAuth, fetchOrderDetails, showToast]);
    const handleUpdateStatus = async (type) => { setUpdatingStatus(true); try { const config = { headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` } }; let response; let successMessage = ''; if (type === 'pay') { const paymentResult = { id: `mock_id_${Date.now()}`, status: 'COMPLETED', update_time: new Date().toISOString(), email_address: order.user?.email }; response = await axios.put(`${API_BASE_URL}/api/orders/${id}/pay`, paymentResult, config); successMessage = t('adminOrdersPage.paidSuccess'); } else if (type === 'deliver') { response = await axios.put(`${API_BASE_URL}/api/orders/${id}/deliver`, {}, config); successMessage = t('adminOrdersPage.deliveredSuccess'); } setOrder(response.data); showToast(successMessage, 'success'); } catch (err) { const errorMessageString = err.response?.data?.message || t('general.errorOccurred'); setError(errorMessageString); showToast(errorMessageString, 'error'); } finally { setUpdatingStatus(false); } };
    const formatPrice = (price) => { return new Intl.NumberFormat(language === 'ar' ? 'ar-EG' : 'en-US', { style: 'currency', currency: t('general.currencyCode') }).format(Number(price || 0)); };
    const formatDateTime = (dateString) => { if (!dateString) return 'N/A'; const options = { year: 'numeric', month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' }; return new Date(dateString).toLocaleDateString(language === 'ar' ? 'ar-EG' : 'en-US', options); };
