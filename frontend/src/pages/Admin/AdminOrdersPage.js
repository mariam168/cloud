...
const AdminOrdersPage = () => {
    ...
    useEffect(() => {
       if (loadingAuth) {
           console.log("AdminOrdersPage: Waiting for AuthContext to load.");
            return;
        }

       console.log("AdminOrdersPage: Checking authentication and admin role.");
        ...
        if (!isAuthenticated || !currentUser || currentUser.role !== 'admin') {
            console.warn('AdminOrdersPage: Unauthorized access attempt. Redirecting.');
            navigate('/');
        ...
        const fetchOrders = async () => {
            ...
            try { 
                ...
                console.log("AdminOrdersPage: Attempting to fetch orders with token:", token);              
                const response = await axios.get(`${API_BASE_URL}/api/orders`, config);
                setOrders(response.data);
                console.log("AdminOrdersPage: Orders fetched successfully.", response.data);
            } catch (err) {
                console.error('AdminOrdersPage: Error fetching orders:', err.response?.data?.message || err.message);
