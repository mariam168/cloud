...
import { Plus, Minus, ShoppingCart, Heart, Loader2, ImageOff, Check } from 'lucide-react';
...
const ProductDetails = () => {
    ...
    const finalPrice = useMemo(() => selectedSku?.price ?? product?.basePrice ?? 0, [selectedSku, product]);
            ...
            <div className="container mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-16">
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-16">
                    <div className="lg:sticky lg:top-24 self-start">
                        <div className="w-full aspect-square bg-gray-100 rounded-xl flex items-center justify-center overflow-hidden mb-4 border">
                            <img src={`${API_BASE_URL}${mainDisplayImage || product.mainImage}`} alt={product.name} className="w-full h-full object-cover transition-opacity duration-300" />
                        </div>
                         {allImages.length > 0 && (
                    ...
                    <div className="flex flex-col space-y-8">
                        <div>
                            <p className="text-sm font-semibold text-gray-500 uppercase">{product.category.name}</p>
                            <h1 className="text-4xl lg:text-5xl font-extrabold text-gray-900 mt-2">{product.name}</h1>
                        </div>
                        
                        <p className="text-4xl font-bold text-gray-800">{formatPrice(finalPrice)}</p>
                        
                        <div className="space-y-6">
                            {variations.map((variation) => (
                                <div key={variation._id}>
                                    <h3 className="font-semibold mb-3 text-lg text-gray-800">{variation.name_en}</h3>
                                    <div className="space-y-4">
                                        {variation.options.map((option) => (
                                            <div key={option._id}>
                                                ...
                                                >
                                                    {variation.name_en.toLowerCase() === 'color' && (
                                                        <span style={{ backgroundColor: colorMap[option.name_en.toLowerCase()] || '#ccc' }} className={`w-8 h-8 block rounded-full border-2 ${activeOption?._id === option._id ? 'ring-2 ring-offset-2 ring-blue-500 border-white' : 'border-gray-200'}`}></span>
                                                    )}
                                                    <h4 className={`text-md font-medium ${activeOption?._id === option._id ? 'text-blue-600' : 'text-gray-600'}`}>{option.name_en}</h4>
                                                </div>
                                                        ...
                                                        {option.skus.map((sku) => {
                                                            ...
                                                            return (
                                                                <button 
                                                                    ...
                                                                    className={`px-4 py-2 border rounded-lg transition-colors text-sm font-semibold ${isSelected ? 'bg-gray-900 text-white border-gray-900' : 'bg-white hover:border-gray-400'}`}
                                                                >
                                                                    {sku.name_en}
                                                                </button>
                                                            );
                        ...
                        <div className="pt-6 border-t">
                            <div className="flex items-center gap-4">
                                <div className="flex items-center border rounded-lg p-1">
                                    <button onClick={() => setQuantity(q => Math.max(1, q - 1))} className="p-2 text-gray-500 hover:bg-gray-100 rounded-md"><Minus size={16}/></button>
                                    <span className="px-5 font-bold text-lg w-12 text-center">{quantity}</span>
                                    <button onClick={() => setQuantity(q => q + 1)} className="p-2 text-gray-500 hover:bg-gray-100 rounded-md"><Plus size={16}/></button>
                                </div>
                                <button onClick={handleAddToCart} disabled={isAddToCartDisabled} className="flex-1 bg-gray-900 text-white p-3 rounded-lg flex items-center justify-center gap-2 hover:bg-gray-700 disabled:bg-gray-400 font-semibold text-lg transition-colors">
                            ...
                            {selectedSku && selectedSku.stock !== undefined && (
                                <p className={`mt-3 text-sm font-semibold ${selectedSku.stock > 5 ? 'text-green-600' : (selectedSku.stock > 0 ? 'text-orange-600' : 'text-red-600')}`}>
                                    {selectedSku.stock > 0 ? `${selectedSku.stock} ${t('cartPage.inStock')}` : t('productDetailsPage.outOfStock')}
                                </p>
                            )}
                        </div>

                        <div className="pt-6 border-t">
                            <h3 className="font-bold mb-3 text-xl text-gray-800">{t('productDetailsPage.specifications')}</h3>
                            <dl className="space-y-3">
                                {product.attributes.map(attr => (
                                    <div key={attr.key_en} className="grid grid-cols-3 gap-4 text-sm">
                                        <dt className="font-medium text-gray-500">{attr.key_en}</dt>
                                        <dd className="col-span-2 text-gray-800">{attr.value_en}</dd>
                                    </div>
                                ))}
                            </dl>
                        </div>
