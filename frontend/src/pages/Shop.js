            ...
            <div className="mx-auto max-w-screen-xl">
                <div className="flex flex-col md:flex-row gap-8">
                    <div className="w-full md:w-1/4 bg-white dark:bg-gray-800 rounded-lg shadow-md p-6">
                        ...
                        <div className="mt-8">
                            <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-3">
                                {t.categories?.["All Categories"] || 'All Categories'}
                            </h3>
                            <ul>
                               {t.categories && Object.keys(t.categories).map(categoryKey => (
                                    <li
                                        key={categoryKey}
                                        className={`cursor-pointer py-2 text-gray-700 dark:text-gray-300 hover:text-blue-600 dark:hover:text-blue-400 ${selectedCategory === categoryKey ? 'font-bold text-blue-600 dark:text-blue-400' : ''}`}
                                        onClick={() => setSelectedCategory(categoryKey)}
                                    >
                                        {t.categories?.[categoryKey] || categoryKey} ({products.filter(p => categoryKey === 'All Categories' || p.category === categoryKey).length})
                                    </li>
                                ))}
                            </ul>
                        </div>
                    ...
                    <div className="w-full md:w-3/4">
                        <div className="flex flex-col sm:flex-row justify-between items-center bg-white dark:bg-gray-800 rounded-lg shadow-md p-4 mb-6">
                            ...
                            <div className="text-gray-700 dark:text-gray-300">
                                {t.shopPage?.showingItems || 'Showing'}: {sortedProducts.length} {t.shopPage?.items || 'items'}
                            </div>
                        </div>
